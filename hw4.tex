...
We have calculated and visualize $\chi^2$--summary according to \cite{CCODA_Mirkin}: we put Pearson's indices outside the parenthesis and it's squared value inside the parenthesis. Such presentation provides mush more information than usual $\chi^2$-statistics.

As we can see from the Table \ref{tbl:chi2_table_ch_timegroup}, the largest contribution (almost 30\%!) belongs to dependence between \textsf{World channel} and \textsf{1st time-group} (most recent articles). We can sayt that dependence is positive because the Peason's index is positive.

In contrast, in Table \ref{tbl:chi2_table_ch_timegroup} all values are approximately equal to each other, there is no outstanding dependences. So, our first suggestion about dependence between \textsf{Lifestyle} and \textsf{Weekend} is not confirmed.
